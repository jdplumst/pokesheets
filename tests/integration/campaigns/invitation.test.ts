...
describe("invitations repository", () => {
	...
	describe("fetch invitations", () => {
		...
		it("should fetch only pending invitations for the specified user", async () => {
			const result = await fetchInvitationsRepository(
				testDb.db,
	...
	describe("fetch campaign invitation by id", () => {
		...
		it("should fetch the correct invitation by id for the specified user", async () => {
			const result = await fetchInvitationByIdRepository(
				testDb.db,
				TEST_USERS.invitedUser.id,
				TEST_INVITATION.id,
			);
	...
	describe("accept campaign invitation", () => {
		...
		it("should update the invitation status to accepted", async () => {
			const result = await acceptInvitationRepository(
				testDb.db,
				TEST_INVITATION.id,
			);
		...
		it("should return null for non-existent invitation", async () => {
			const nonExistentId = faker.string.uuid();
			const result = await acceptInvitationRepository(testDb.db, nonExistentId);

			expect(result).toBeNull();
		});

		it("should handle already accepted invitation", async () => {
			await acceptInvitationRepository(testDb.db, TEST_INVITATION.id);
