				...
				{import.meta.env.PROD ? (
					<div className="space-y-4">
						<Button
							...
							onClick={() =>
								authClient.signIn.social({
									provider: "discord",
									callbackURL: `${import.meta.env.VITE_CLIENT_URL ?? "http://localhost:3000"}/campaigns`,
									errorCallbackURL: `${import.meta.env.VITE_CLIENT_URL ?? "http://localhost:3000"}/error`,
									fetchOptions: {
						...
						>
							...
							{loading ? <Spinner /> : "Sign in with Discord"}
						</Button>
					...
					<Card>
						...
						<CardContent className="gap-2 flex flex-col">
							{DEV_USERS.map((d) => (
								<Item key={d.id} variant="outline" className="flex flex-col">
									<ItemContent>{d.name}</ItemContent>
									<ItemActions>
										<Button
											variant="default"
											disabled={loading}
